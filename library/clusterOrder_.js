...
exports.clusterKnn = (request, response) => {
  let parameters      = request.body;
  let factors         = parameters.factors;
  let order           = parameters.order;
  let vehicle         = parameters.vehicle;
  ...
  //let clusterNo = parseInt(orderLatLng.length/factorDrops);
  console.log("orderLatLng",orderLatLng);
  returnArray  = new kNN(orderLatLng);
  console.log("returnArray",returnArray);
  return response.status(200).json({responseCode : 1, responseMessage : "Clustering successfully", responseData : [] })
